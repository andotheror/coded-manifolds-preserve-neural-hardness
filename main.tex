...
\[
R=O(n^\alpha),\qquad \alpha<\frac12.
\]
...
We first form a smooth abstract curve through the orthogonal vertices $\{e_z:z\in\Ftwo^m\}\subset\R^{2^m}$ in reflected Gray order. Every point, tangent, and local derivative is supported on at most three adjacent messages. We then map $e_z$ to a systematic codeword
\[
C(z)=\bigl(z,\mathbf1\{\langle R_1,z\rangle=1\},\ldots,
\mathbf1\{\langle R_N,z\rangle=1\}\bigr),
\]
where $R_1,\ldots,R_N$ form a constant-bias multiset in $\Ftwo^m$. Such multisets of linear size have deterministic polynomial-time constructions \citep{naor1993smallbias,tashma2017balanced}. For a uniform character $R$ and any real signed measure $w=(w_z)$ of total mass zero, character orthogonality gives
\begin{equation}
...
Let $e_z\in\R^K$ be the basis vector indexed by $z\in\Ftwo^m$. Start from the closed polygon that visits $e_{z_0},\ldots,e_{z_{K-1}}$ in Gray order. Truncate a fixed fraction of every edge and replace every corner by the same smooth planar rounding inside
\[
\operatorname{conv}\{e_{z_{i-1}},e_{z_i},e_{z_{i+1}}\}.
\]
The rounding agrees with both adjacent straight segments to every derivative order. Appendix~\ref{app:abstract} gives an explicit tangent-angle construction.
...
Fix a sufficiently small constant $\varepsilon_0>0$. An $\varepsilon_0$-biased multiset $\mathcal R\subset\Ftwo^m$ satisfies
\[
\left|\frac1{|\mathcal R|}\sum_{R\in\mathcal R}(-1)^{\langle R,a\rangle}\right|\leq\varepsilon_0
...
\leq\frac{\varepsilon N(s-1)}4\|w\|_2^2.
\]
Choosing $\varepsilon\leq1/(2(s-1))$ gives constant lower and upper bounds without a net or union bound. The systematic coordinates add a nonnegative quadratic form and at most $ms\|w\|_2^2$. Choosing $r$ small enough makes $N\geq C_sm$. Appendix~\ref{app:rip} gives the details, including the harmless truncation of the last copy.

\begin{proposition}[Critical reach]
\label{prop:reach}
For the code in Lemma~\ref{lem:rip} with $s=12$, the curve $M_n$ is a closed $C^\infty$ embedding and
\[
c\sqrt n\leq\reach(M_n)\leq\sqrt n.
\]
\end{proposition}

\begin{proof}[Proof sketch]
Let $p,q\in\Gamma_m$, let $w=q-p$, and let $v$ span the tangent at $p$. The vectors $w$ and $w-tv$ have total mass zero and constant support for every $t\in\R$. Lemma~\ref{lem:rip} gives
\[
\|Fw\|_2^2\geq cN\|w\|_2^2,
\qquad
\dist(Fw,\operatorname{span}Fv)
\leq C\sqrt N\dist(w,\operatorname{span}v).
\]
Substitution into \eqref{eq:federer} and Lemma~\ref{lem:abstract} give $\reach(M_n)\geq c\sqrt N=\Omega(\sqrt n)$. The same lower inequality makes $T$ injective on the curve and regular on every tangent.
...
\label{lem:gray}
For $X\sim\cD_n$, the probability that $X$ lies on a bad piece is $O(2^{-t})$. Conditioned on the piece type and on being good, the normalized prefix
\[
Z(X)=\left(\frac{X_1-a}{1-2a},\ldots,
\frac{X_d-a}{1-2a}\right)
\]
...
\begin{proof}
The probability that either draw is bad is $O(2^{-t})$. Conditioned on two good draws, their decoded prefixes are independent and uniform by Lemma~\ref{lem:gray}. Two vectors in $\Ftwo^d$ fail to be linearly independent only if one is zero or they are equal, an event of probability at most $3\cdot2^{-d}$. Otherwise the map
\[
S\longmapsto
(\langle S,Z(X)\rangle,\langle S,Z(X')\rangle)
\]
has rank two and is uniform on $\Ftwo^2$. Equation~\eqref{eq:parity} finishes the proof. Since $d,t=\Omega(n)$, the failure probability is exponentially small.
\end{proof}
...
The random-function mixture first draws $X\sim D$ and then draws a fresh uniform $h\in\cH$ to set the label $h(X)$. For completeness, if $\phi[h]=\mathbb E_X\phi(X,h(X))$, the same calculation as in Lemma C.3 of \citet{chen2022noisefree} gives
\[
\operatorname{Var}_{h\sim\operatorname{Unif}(\cH)}\phi[h]\leq2\eta.
\]
...
\[
b_\delta(s)=Z_\delta^{-1}\exp\left(-\frac{\delta}{s(1-s)}\right),\qquad 0<s<1,
\]
and set it to zero at the endpoints, where $Z_\delta$ normalizes its integral to one. This is a positive symmetric $C^\infty$ function that is flat at zero and one. As $\delta\downarrow0$, it converges to the constant function one on compact subintervals of $(0,1)$. Write $b=b_\delta$. For a length $L>0$, define
\[
\beta(s)=\theta\int_0^{s/L}b(r)\,dr,
\qquad 0\leq s\leq L.
\]
Let $U(\varphi)$ be the unit vector obtained by rotating $u$ through angle $\varphi$ in the plane spanned by the three local basis vectors, and put
\[
\gamma(s)=p+\int_0^sU(\beta(q))\,dq.
\]
Symmetry gives $U(\beta(s))+U(\beta(L-s))$ parallel to $u+v$. Hence $\gamma(L)-\gamma(0)=c_bL(u+v)$ for a constant $c_b>0$. Choosing $L=\alpha/c_b$ makes the endpoints equal to the two truncated polygon points. For $b\equiv1$, the formula is the circular arc tangent to both edges and lying in the local triangle. Positive smooth flat approximations preserve the two inward-pointing endpoint jets and converge to this circular arc away from the endpoints. Choosing one sufficiently close approximation therefore keeps the open arc in the triangle. This template and all its constants are fixed once and for all.
...
Near the diagonal, let $\gamma$ be any local unit-speed parametrization and let $\kappa_0$ bound curvature. Taylor's theorem gives, for small $|h|$,
\[
\dist(\gamma(s+h)-\gamma(s),\operatorname{span}\gamma'(s))
\leq\frac{\kappa_0}{2}h^2,
\qquad
\|\gamma(s+h)-\gamma(s)\|\geq\frac{|h|}{2}.
\]
The quotient in \eqref{eq:federer} is therefore bounded below by a positive constant near the diagonal. Compactness handles every remaining local pair. The minimum of these finitely many constants and the disjoint-support bound is $r_0>0$.

\section{Small-bias character isometry}
\label{app:rip}

We prove Lemma~\ref{lem:rip}. Let $\mathcal R_0\subset\Ftwo^m$ be an explicit $\varepsilon_0$-biased multiset of size $N_0\leq C_{\varepsilon_0}m$ \citep{naor1993smallbias,tashma2017balanced}. Let $q=\lfloor N/N_0\rfloor$. By taking $r$ sufficiently small, $q\geq1/\varepsilon_0$. Repeat $\mathcal R_0$ exactly $q$ times and append any $N-qN_0<N_0$ elements of one further copy. For every $a\ne0$, the resulting empirical character average has absolute value at most
\[
\frac{qN_0\varepsilon_0+(N-qN_0)}N\leq\varepsilon_0+\frac1q\leq2\varepsilon_0.
\]
Write $\varepsilon=2\varepsilon_0$ for this final bias. Let $A\in\{0,1\}^{N\times K}$ be the parity part of $F$, so
\[
A_{\ell,z}=\mathbf1\{\langle R_\ell,z\rangle=1\}.
\]
For $\chi_R(z)=(-1)^{\langle R,z\rangle}$,
\[
\mathbf1\{\langle R,z\rangle=1\}=\frac{1-\chi_R(z)}2.
\]
If $\sum_z w_z=0$, then
\[
\sum_z w_z\mathbf1\{\langle R,z\rangle=1\}
=-\frac12\sum_z w_z\chi_R(z).
\]
For a zero-mass vector $w$, expansion over the empirical law of the $R_\ell$ gives
\begin{align}
\frac1N\|Aw\|_2^2
...
\[
\left|\frac1N\|Aw\|_2^2-\frac14\|w\|_2^2\right|
\leq\frac\varepsilon4\sum_{z\ne z'}|w_zw_{z'}|
\leq\frac{\varepsilon(s-1)}4\|w\|_2^2.
\]
Choose $\varepsilon\leq1/(2(s-1))$. Then, simultaneously for every such $w$,
\begin{equation}
...
\[
|(Bw)_j|\leq\|w\|_1\leq\sqrt s\|w\|_2,
\qquad
\|Bw\|_2^2\leq ms\|w\|_2^2.
\]
Since $F=(B,A)$, the lower bound in \eqref{eq:parity-rip} remains valid and the systematic part changes only the upper constant when $m=O(N)$. This proves \eqref{eq:rip}.

\section{Reach transfer in detail}
\label{app:reach}

Let $q=1-2a=1/2$. The derivative of $T$ is $qF$. Fix distinct $p,u\in\Gamma_m$, let $w=u-p$, and let $v\ne0$ span $T_p\Gamma_m$. By Lemma~\ref{lem:abstract}, $w$ has support at most six and $v$ has support at most three. Both have coordinate sum zero, and every $w-\lambda v$ has support at most six. Lemma~\ref{lem:rip} yields constants $c_0,C_0>0$ such that
\[
\|qFw\|_2^2\geq c_0N\|w\|_2^2
\]
and
\begin{align*}
...
\[
\frac{\|T(u)-T(p)\|_2^2}
{2\dist(T(u)-T(p),T_{T(p)}M_n)}
\geq
\frac{c_0}{C_0}\sqrt N
\frac{\|u-p\|_2^2}{2\dist(u-p,T_p\Gamma_m)}.
\]
Taking the infimum and using \eqref{eq:federer} gives
\[
\reach(M_n)\geq\frac{c_0}{C_0}\sqrt N\reach(\Gamma_m)
\geq c\sqrt n.
\]
...
Write the reflected Gray word at index $i\in\{0,\ldots,2^m-1\}$ as
\[
G(i)=i\mathbin{\mathrm{xor}}(i\!\gg\!1).
\]
...
\]
Every block loses the same number of indices for each fixed piece type. Conditional on that type and goodness, its prefix is therefore uniform on $\Ftwo^d$. Mixing the two types preserves uniformity and proves Lemma~\ref{lem:gray}.

Let $s_j(u)$ denote the abstract parametrization of type $j\in\{\mathrm{straight},\mathrm{corner}\}$ and index $i$. Its speed lies in a universal interval $[c_0,C_0]$. Since its tangent has total mass zero and support at most three, Lemma~\ref{lem:rip} gives
\[
c\sqrt n\leq\|(T\circ s_j)'(u)\|_2\leq C\sqrt n.
\]
There are $2K$ parameter intervals, each selected with probability $1/(2K)$. The total image length is between $cK\sqrt n$ and $CK\sqrt n$. Dividing parameter density by speed and comparing with inverse total length shows that $d\cD_n/d\operatorname{UnifArc}$ lies between two universal positive constants.

\section{Pairwise independence and learning reduction}
\label{app:sq}

Let $E$ be the event that both independent draws lie on good pieces and their decoded prefixes are nonzero and distinct. Lemma~\ref{lem:gray} gives
\[
\Pr(E^c)\leq C2^{-t}+2\cdot2^{-d}+2^{-d}
\leq\exp(-\rho n).
\]
Over $\Ftwo$, two nonzero vectors are linearly dependent exactly when they are equal. Thus, on $E$, the linear map from $S\in\Ftwo^d$ to its two parity bits has rank two. Uniform $S$ produces two independent uniform signs. This proves Lemma~\ref{lem:independence}.

We also spell out the final validation query. Let $\widehat h$ be the learner's output and clip it pointwise to $[-1,1]$. Query
\[
\phi(x,y)=\frac14(\widehat h(x)-y)^2\in[0,1].
\]
For a learned deterministic target, $\mathbb E\phi\leq1/64$. Under the random-function mixture, condition on a good input with a nonzero decoded prefix. Its label is an independent uniform sign, and
\[
\mathbb E_y(\widehat h(x)-y)^2=1+\widehat h(x)^2\geq1.
\]
...
We use Corollary~5.3 of \citet{chen2022noisefree}: an efficient learner for polynomial-size one-hidden-layer ReLU networks under the uniform law on $\{0,1\}^d$, to constant squared error, yields an efficient algorithm for polynomial-modulus LWR.
